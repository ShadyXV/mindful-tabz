  ...
  if (total === 0) {
    return (
      <section className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
        <h3 className="text-sm font-black text-slate-300 uppercase tracking-widest mb-4">Tracked sites</h3>
  ...
  return (
    <section className="bg-slate-900 border border-slate-800 rounded-3xl p-6">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h3 className="text-sm font-black text-slate-300 uppercase tracking-widest">Tracked sites</h3>
      ...
      <div className="grid grid-cols-[9rem_1fr] gap-5 items-center">
        <div className="relative w-36 h-36">
          ...
          <div className="absolute inset-0 flex items-center justify-center text-center">
            <span className="text-2xl font-black text-white tracking-tighter">{formatTime(total)}</span>
          </div>
        </div>
          ...
          {data.slice(0, 5).map((entry, index) => {
            ...
            return (
              <div key={entry.domain} className="grid grid-cols-[auto_1fr_auto] items-center gap-2 min-w-0">
                ...
                />
                <span className="text-sm font-bold text-slate-300 truncate">{entry.domain}</span>
...
export function ScreenTimeView({ sites, screenTime, onQuickAdd }: ScreenTimeViewProps) {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <section className="bg-indigo-600 border border-indigo-500/30 p-6 rounded-3xl shadow-xl shadow-indigo-950/20">
        <div className="flex items-start justify-between gap-4 mb-6">
          <div>
            <p className="text-indigo-100/80 font-black uppercase tracking-[0.2em] text-xs mb-2">Today</p>
            <h2 className="text-4xl font-black text-white tracking-tighter">{formatTime(totalTime)}</h2>
          </div>
          <div className="bg-indigo-400/20 p-3 rounded-2xl">
            <Clock3 className="w-7 h-7 text-indigo-50" />
          </div>
        </div>

        <div className="grid grid-cols-2 gap-3">
          <div className="bg-indigo-950/25 rounded-2xl p-4">
            <p className="text-indigo-100/70 text-[10px] font-black uppercase tracking-widest mb-1">Tracked sites</p>
            <p className="text-2xl font-black text-white">{formatTime(trackedTime)}</p>
          </div>
          <div className="bg-indigo-950/25 rounded-2xl p-4">
            <p className="text-indigo-100/70 text-[10px] font-black uppercase tracking-widest mb-1">Untracked sites</p>
            <p className="text-2xl font-black text-white">{formatTime(untrackedTime)}</p>
          </div>
        </div>
      </section>
      ...
      <section className="grid grid-cols-1 gap-3">
        <h3 className="text-sm font-black text-slate-500 uppercase tracking-widest px-2">Websites visited</h3>
        {domainTimes.length > 0 ? (
          domainTimes.map(entry => (
            <div
              key={entry.domain}
              className="bg-slate-900 p-5 rounded-2xl border border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-slate-700 transition-colors"
            >
              <div className="flex items-center gap-4 min-w-0">
                <div className={`p-3 rounded-xl ${entry.isTracked ? 'bg-indigo-600/20' : 'bg-slate-800'}`}>
                  <Globe className={`w-6 h-6 ${entry.isTracked ? 'text-indigo-400' : 'text-slate-400'}`} />
                </div>
                <div className="min-w-0">
                  <h4 className="font-bold text-lg text-white truncate">{entry.domain}</h4>
